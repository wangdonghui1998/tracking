...
class LBHinge(nn.Module):
    ...
    在下限上使用“hinge”的损失。 这意味着，对于标签值小于阈值的样本，如果预测值也小于阈值，则损失为零。
    args:
...
class TrackingClassificationAccuracy(nn.Module):
    """ Estimates tracking accuracy by computing whether the peak of the predicted score map matches with the target
        location.   通过计算预测得分图的峰值是否与目标位置匹配来估计跟踪精度。
    """
    ...
    def forward(self, prediction, label, valid_samples=None):   #prediction=[1,1,18,18], label[1,1,18,18]
        prediction_reshaped = prediction.view(-1, prediction.shape[-2] * prediction.shape[-1])  #(1,18*18)
        label_reshaped = label.view(-1, label.shape[-2] * label.shape[-1])    # (1,18*18)
