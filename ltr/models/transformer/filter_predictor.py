...
class FilterPredictor(nn.Module):
    ...
    def predict_filter(self, train_feat, test_feat, train_label, train_ltrb_target, *args, **kwargs):
        ...
        :param train_ltrb_target: [2,1,4,18,18]
        :return:
        """
        if train_feat.dim() == 4:
        ...
        test_pos = self.get_positional_encoding(test_feat) # Nf_te, Ns, C, H, W   只用到了feat的shape   [1,1,256,18,18]
        train_pos = self.get_positional_encoding(train_feat) # Nf_tr, Ns, C, H, W   [2,1,256,18,18]

        test_feat_seq = test_feat.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1) # Nf_te*H*W, Ns, C   [324,1,256] permute:维度交换顺序，flatten：从第dim个维度开始展开，将后面的维度转化为一维
        train_feat_seq = train_feat.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1) # Nf_tr*H*W, Ns, C   [648,1,256]
        train_label_seq = train_label.permute(1, 0, 2, 3).flatten(1).permute(1, 0).unsqueeze(2) # Nf_tr*H*W,Ns,1   [648,1,1]
        train_ltrb_target_seq_T = train_ltrb_target.permute(1, 2, 0, 3, 4).flatten(2) # Ns,4,Nf_tr*H*W   [1,4,648]

        test_pos = test_pos.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1)    #[324,1,256]  与test_feat_seq对应
        train_pos = train_pos.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1)  #[648,1,256]  与train_feat_seq对应

        fg_token = self.query_embed_fg.weight.reshape(1, 1, -1)  #用于查找的token   [1,1,256]
        train_label_enc = fg_token * train_label_seq    #[648,1,256]

        train_ltrb_target_enc = self.box_encoding(train_ltrb_target_seq_T).permute(2,0,1) # Nf_tr*H*H,Ns,C   [648,1,256]
        ...
        if self.use_test_frame_encoding:
            ...
            feat = torch.cat([train_feat_seq + train_label_enc + train_ltrb_target_enc, test_feat_seq], dim=0)  #transformer的encoder中沿第一个维度concate   [972,1,256]

        pos = torch.cat([train_pos, test_pos], dim=0)  # [972,1,256]
        # transformer encoder-decoder
        output_embed, enc_mem = self.transformer(feat, mask=None, query_embed=self.query_embed_fg_decoder.weight, pos_embed=pos)   #output_embed.shape=[1,1,,1,256]  enc_mem.shape=[972,1,256]

        enc_opt = enc_mem[-h*w:].transpose(0, 1)  #[1,324,256]  transpose(Tensor,dim0,dim1)是pytorch中的ndarray矩阵进行转置的操作
        dec_opt = output_embed.squeeze(0).transpose(1, 2)  #[1,256,1]
