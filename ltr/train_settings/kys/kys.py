...
def run(settings):
    ...
    loader_val = LTRLoader('val', dataset_val, training=False, batch_size=settings.batch_size,
                           num_workers=settings.num_workers,
                           shuffle=False, drop_last=True, epoch_interval=5, stack_dim=1)   #仅声明
    ...
    loss_weight = {'test_clf': 1.0*500, 'test_clf_orig': 50, 'is_target': 0.1*500, 'is_target_after_prop': 0.1*500}  #变量声明
    ...
    actor = actors.KYSActor(net=net, objective=objective, loss_weight=loss_weight,
                            dimp_jitter_fn=dimp_jitter_fn)      #仅声明

    optimizer = optim.Adam([{'params': actor.net.predictor.parameters(), 'lr': 1e-2}],  #变量声明
                           lr=1e-2)

    lr_scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=15, gamma=0.2)    #变量声明
    ...
    trainer.train(40, load_latest=True, fail_safe=True)  #开始调用
