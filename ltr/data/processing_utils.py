...
def sample_target_adaptive(im, target_bb, row_bb, search_area_factor, output_sz, mode: str = 'replicate',
                           max_scale_change=None, mask=None):
    # (frames, box_extract,5,(288,288),insid_emajor,1.5)
    ...
    提取以区域search_area_factor^2的target_bb框为中心的裁剪。如果裁剪区域包含区域在图像外部，它被移动，以便它位于图像内部。
    此外，如果裁剪区域超出图像大小，则返回适合图像的较小裁剪。
    args:
        im - Input numpy image to crop.
        target_bb - target box [x, y, w, h]
        search_area_factor - Ratio of crop size to target size 5
        output_sz - (float) Size to which the extracted crop is resized (always square). If None, no resizing is done. （288，288）
        mode - If 'replicate', the boundary pixels are replicated in case the search region crop goes out of image.
               ...
               If 'inside_major', the search region crop is shifted/shrunk to fit completely inside one axis of the image.  insid_emajor
        max_scale_change - Maximum allowed scale change when performing the crop (only applicable for 'inside' and 'inside_major')   1.5
    ...
    x , y ,w ,h = row_bb.tolist()

    # Crop image
    crop_sz_x, crop_sz_y = (output_sz * (
                target_bb[2:].prod() / output_sz.prod()).sqrt() * search_area_factor).ceil().long().tolist()  #ceil()上取整，long()转化为长整型 。（830，830）

    # Get new sample size if forced inside the image
    if mode == 'inside' or mode == 'inside_major':
        # Calculate rescaling factor if outside the image  如果超出图像范围，计算缩放因子
        ...
        crop_sz_x = math.floor(crop_sz_x / rescale_factor)  #floor()下取整
        crop_sz_y = math.floor(crop_sz_y / rescale_factor)
    #crop_sz_x，crop_sz_y : target_box *5 大小的区域，（x1,y1）区域左上角坐标 (x2,y2)右下角坐标

    if crop_sz_x < 1 or crop_sz_y < 1:
        raise Exception('Too small bounding box.')

    x1 = round(bbx + 0.5 * bbw - crop_sz_x * 0.5)  #round()四舍五入
    x2 = x1 + crop_sz_x
    ...
    # rec1 = patches.Rectangle((x1, y1), crop_sz_x, crop_sz_y, linewidth=2, edgecolor='y', facecolor='None')
    # ax.add_patch(rec1)
    # rec2 = patches.Rectangle((bbx, bby), bbw, bbh, linewidth=2, edgecolor='r', facecolor='None')
    # ax.add_patch(rec2)
    # rec3 = patches.Rectangle((x, y), w, h, linewidth=2, edgecolor='g', facecolor='None')
    # ax.add_patch(rec3)
    # vis.matplot(plt)

    # Move box inside image  如果上面的框有超出原始图像的，即裁剪不到这些区域了，则将框进行移动
    ...
    #(x1,y1) 经过移动后的框的左上角位置 ， (x2,y2) 经过移动后的框的右下角位置
    # rec4 = patches.Rectangle((x1, y1), x2-x1, y2-y1, linewidth=2, edgecolor='#FFA500', facecolor='None')
    # ax.add_patch(rec4)
    # vis.matplot(plt)
    ...
    # Resize image
    im_out = cv.resize(im_crop_padded, tuple(output_sz.long().tolist()))  #cv.resize（） 放大或者缩小图像到某一个尺寸，像素会糊一点
...
def sample_target_adaptive2(im, target_bb, row_bb, search_area_factor, output_sz, mode: str = 'replicate',
                           max_scale_change=None, mask=None):
    # (frames, box_extract,5,(288,288),insid_emajor,1.5)
    """ Extracts a crop centered at target_bb box, of area search_area_factor^2. If the crop area contains regions
    outside the image, it is shifted so that the it is inside the image. Further, if the crop area exceeds the image
    ...
    # Move box inside image  如果上面的框有超出原始图像的，即裁剪不到这些区域了，则将框进行移动
    shift_x = max(0, -x1) + min(0, im_w - x2)  #shift_x>0 左边宽  shift_x<0 右边宽
    x1 += shift_x
    x2 += shift_x
    ...
    crop_box = torch.Tensor([x1, y1, x2 - x1, y2 - y1])
    # 保存resize的参数
    im_resize = crop_box[2] / output_sz[0]

    if mask is None:
        return im_out, crop_box,im_resize
    else:
        return im_out, crop_box, mask_out,im_resize
...
def transform_box_to_crop(box: torch.Tensor, crop_box: torch.Tensor, crop_sz: torch.Tensor) -> torch.Tensor:
    """ Transform the box co-ordinates from the original image co-ordinates to the co-ordinates of the cropped image将框坐标从原始图像坐标转换为裁剪图像的坐标
    args:
        box - the box for which the co-ordinates are to be transformed
        crop_box - bounding box defining the crop in the original image
        crop_sz - size of the cropped image
            box: groundtruth的坐标，crop_box:实际裁剪区域box坐标大小, crop_sz:裁剪大小
    returns:
        torch.Tensor - transformed co-ordinates of box_in
    """
...
def target_image_crop(frames, box_extract, box_gt, search_area_factor, output_sz, mode: str = 'replicate',
    ...
    每个帧，以box_extract（jittered_anno）为中心裁剪一个方形区域，面积为search_area_factor（5.0）^2 * box_extract。
    如果裁剪区域包含图像外部的区域，则会移动/缩小裁剪区域，使其完全适合图像内部。然后将提取的crop resize成output_sz大小。
    此外，box 的box_gt被转换为图像裁剪坐标。

    args:
        frames - list of frames
        box_extract - list of boxes of same length as frames. The crops are extracted using anno_extract. jittered box的列表
        box_gt - list of boxes of same length as frames. The co-ordinates of these boxes are transformed from
                    image co-ordinates to the crop co-ordinates  box_gt的列表
        search_area_factor - The area of the extracted crop is search_area_factor^2 times box_extract area  5
        output_sz - The size to which the extracted crops are resized  把提取的crops resize成多大的     288
        mode - If 'replicate', the boundary pixels are replicated in case the search region crop goes out of image.在搜索区域裁剪超出图像范围时复制边界像素。
               If 'inside', the search region crop is shifted/shrunk to fit completely inside the image. 搜索区域裁剪将移动/缩小以完全适合图像内部。
               If 'inside_major', the search region crop is shifted/shrunk to fit completely inside one axis of the image.搜索区域裁剪将被移动/缩小，以完全适应图像的一个轴。
               'insid_emajor'
        max_scale_change - Maximum allowed scale change when performing the crop (only applicable for 'inside' and 'inside_major') 执行裁剪时允许的最大缩放变化  1.5
    ...
    if isinstance(output_sz, (float, int)):
        output_sz = (output_sz, output_sz)   #(288,288)

    if masks is None:
        frame_crops_boxes = [sample_target_adaptive(f, a, b,search_area_factor, output_sz, mode, max_scale_change)
                             for f, a ,b in zip(frames, box_extract, box_gt)]  #(frames, box_extract,5,(288,288),insid_emajor,1.5)
...
def target_image_crop2(frames, box_extract, box_gt, search_area_factor, output_sz, mode: str = 'replicate',
                      max_scale_change=None, masks=None):
    """ For each frame in frames, extracts a square crop centered at box_extract, of area search_area_factor^2
    times box_extract area. If the crop area contains regions outside the image, it is shifted / shrunk so that it
    completely fits inside the image. The extracted crops are then resized to output_sz. Further, the co-ordinates of
    ...
    if masks is None:
        frame_crops_boxes = [sample_target_adaptive2(f, a, b,search_area_factor, output_sz, mode, max_scale_change)
                             for f, a ,b in zip(frames, box_extract, box_gt)]  #(frames, box_extract,5,(288,288),insid_emajor,1.5)

        frames_crop, crop_boxes,im_resize_factor = zip(*frame_crops_boxes)
    else:
        frame_crops_boxes_masks = [
            sample_target_adaptive2(f, a, search_area_factor, output_sz, mode, max_scale_change, mask=m)
            for f, a, m in zip(frames, box_extract, masks)]

        frames_crop, crop_boxes, masks_crop,im_resize_factor = zip(*frame_crops_boxes_masks)

    crop_sz = torch.Tensor(output_sz)

    # find the bb location in the crop  找到裁剪中的bb位置
    box_crop = [transform_box_to_crop(bb_gt, crop_bb, crop_sz)       #box_gt: groundtruth的坐标，crop_bb:实际裁剪区域box坐标大小, crop_sz:裁剪大小
                for bb_gt, crop_bb in zip(box_gt, crop_boxes)]

    if masks is None:
        return frames_crop, box_crop, crop_boxes, im_resize_factor
    else:
        return frames_crop, box_crop, masks_crop,crop_boxes,im_resize_factor
...
def gauss_2d(sz, sigma, center, end_pad=(0, 0), density=False):   #center [2,2],sigma:0.9 sz:torch.tensor[18,18]
    #sz=18, sigma=0.9, center, end_pad=（0，0）,
...
def gaussian_label_function(target_bb, sigma_factor, kernel_sz, feat_sz, image_sz, end_pad_if_even=True, density=False,
                            uni_bias=0):
    #target_bb=[x,y,w,h]，sigma_factor=0.05, kernel_sz=1, feat_sz=18, image_sz=288
    """Construct Gaussian label function. 高斯标签生成函数"""
    ...
    image_sz = torch.Tensor(image_sz)   #(288,288)
    feat_sz = torch.Tensor(feat_sz)     #(18,18)

    target_center = target_bb[:, 0:2] + 0.5 * target_bb[:, 2:4]     #[30,2]
    target_center_norm = (target_center - image_sz / 2) / image_sz    #标准化，input-mean(input)/std(input)   [30,2]

    center = feat_sz * target_center_norm + 0.5 * \
             torch.Tensor([(kernel_sz[0] + 1) % 2, (kernel_sz[1] + 1) % 2])   #[30,2]
