...
class Transform:
    ...
    The following parameters can be supplied with calling the transform object:
        joint [Bool]  -  If True then transform all images/coords/bbox/mask in the list jointly using the same transformation.
                      ...
                      -  如果为True，则使用相同的变换联合变换列表中的所有 images/coords/bbox/mask
                         否则，每个元组（图像、坐标、bbox、掩码）将使用不同的随机滚动进行独立转换。
...
class TransformBase:
    ...
    def __call__(self, **inputs):
        # Split input
        input_vars = {k: v for k, v in inputs.items() if k in self._valid_inputs}  #dict :{'image':, 'bbox':}
        input_args = {k: v for k, v in inputs.items() if k in self._valid_args} #dict:{'new_roll':true}
        ...
        outputs = dict()
        for var_name, var in input_vars.items():    #input_vars:{'image':,  'bbox': } image[0].shape=[1080,1920,3]
            if var is not None:
                transform_func = getattr(self, 'transform_' + var_name)   #getattr()获取对象的属性值,得出来是一个函数名，transform_image()
                if var_name in ['coords', 'bbox']:
                    params = (self._get_image_size(input_vars),) + self._rand_params
                else:
                    params = self._rand_params
                if isinstance(var, (list, tuple)):  #isinstance() 判断一个对象是否是list或tuple类型
                    outputs[var_name] = [transform_func(x, *params) for x in var]   #调用各个用于变换类的transform_image()函数
                else:
...
class ToGrayscale(TransformBase):
    ...
    def transform_image(self, image, do_grayscale):
        if do_grayscale:
            if torch.is_tensor(image):      #is_tensor(image) : 如果image是tensor的话返回true
...
class RandomHorizontalFlip(TransformBase):
    """Horizontally flip image randomly with a probability p.  以概率P随机水平翻转图像"""
    def __init__(self, probability = 0.5):
        super().__init__()
        self.probability = probability
