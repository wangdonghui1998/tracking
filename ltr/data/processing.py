    ...
    def __init__(self, transform=transforms.ToTensor(), train_transform=None, test_transform=None, joint_transform=None):
        ...
        args:
            transform       - The set of transformations to be applied on the images. Used only if train_transform or
                                test_transform is None.  要应用于图像的变换集，仅当train_transform或test_transform为“无”时使用。
            train_transform - The set of transformations to be applied on the train images. If None, the 'transform'
                                argument is used instead.要应用于train图像的变换集。如果无，则使用“transform”参数。
            test_transform  - The set of transformations to be applied on the test images. If None, the 'transform'
                                argument is used instead.要应用于test图像的转换集。如果无，则使用“transform”参数。
            joint_transform - The set of transformations to be applied 'jointly' on the train and test images.  For
                                example, it can be used to convert both test and train images to grayscale.在train和test图像上“联合”应用的变换集。
        """
...
class LSDiMPProcessing(BaseProcessing):
    """ The processing class used for training DiMP. The images are processed in the following way.
    First, the target bounding box is jittered by adding some noise. Next, a square region (called search region )
    centered at the jittered target center, and of area search_area_factor^2 times the area of the jittered box is
    cropped from the image. The reason for jittering the target box is to avoid learning the bias that the target is
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['train', 'test']:
            ...
            # Add a uniform noise to the center pos
            jittered_anno = [self._get_jittered_box(a, s) for a in data[s + '_anno']]
            #crops:裁剪的图片 , boxes：目标相对于裁剪的框的位置大小,crop_boxes：裁剪的框在原图中的坐标,im_resize_factor：裁剪区域——>288的比例
            crops, boxes,crop_boxes,im_resize_factor = prutils.target_image_crop2(data[s + '_images'], jittered_anno, data[s + '_anno'],
                                                     self.search_area_factor, self.output_sz, mode=self.crop_type,
                                                     max_scale_change=self.max_scale_change)

            data[s + '_images'], data[s + '_anno'] = self.transform[s](image=crops, bbox=boxes, joint=False)
            data[s + '_crop_boxes'] = crop_boxes
            data[s + '_im_resize_factor'] = im_resize_factor
...
class KYSProcessing(BaseProcessing):
    """ The processing class used for training KYS. The images are processed in the following way.
        ...
        图像处理方式：
            首先，在目标bbox添加一些噪声抖动
            接着，在图像中裁剪一个以抖动目标中心为中心的方形区域（搜索区域），面积是抖动框面积的平方。抖动目标框的原因是为了避免学习到的偏差一直是目标始终位于搜索区域中心。
            然后，将搜索区域调整为参数output_sz给定的固定大小。每个图像都会生成以目标为中心的高斯标签，这些标签用于计算测试图像上预测分类模型的loss

            通过抖动地面真值框，还为测试图像生成了一组proposal。这些proposal可用于训练边界框估计分支
        """
    def __init__(self, search_area_factor, output_sz, center_jitter_param, scale_jitter_param,
        ...
        """
        super().__init__(*args, **kwargs)
        self.search_area_factor = search_area_factor  #5.0 搜索区域相对于目标的大小
        self.output_sz = output_sz  #288 一个整数，表示搜索区域的大小，搜索区域始终为正方形
        self.center_jitter_param = center_jitter_param      #包含提取搜索区域之前应用于目标中心的抖动量的dict
                                                            # {'train_mode': 'uniform', 'train_factor': 3.0, 'train_limit_motion': False, 'test_mode': 'uniform', 'test_factor': 4.5, 'test_limit_motion': True}
        self.scale_jitter_param = scale_jitter_param        #在提取搜索区域之前，包含要应用于目标大小的抖动量的dict
                                                            #{'train_factor': 0.25, 'test_factor': 0.3}
        self.proposal_params = proposal_params      #proposal生成过程的参数
        self.label_function_params = label_function_params  #label生成过程的参数
        self.min_crop_inside_ratio = min_crop_inside_ratio  #0.1 图像内部的最小裁剪搜索区域数量
    ...
    def __call__(self, data: TensorDict):   #类重载时会调用
        if self.transform['joint'] is not None:
...
class LTRBDenseRegressionProcessing(BaseProcessing):
    """ The processing class used for training ToMP that supports dense bounding box regression.
        用于训练支持密集边界框回归的ToMP的处理类  ,生成bbox的ltrb编码
    """
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor, crop_type='replicate',
        ...
        """
        super().__init__(*args, **kwargs)
        self.search_area_factor = search_area_factor   #5.0 相对于目标大小的搜索区域的大小
        self.output_sz = output_sz  #18（特征大小）*16 搜索区域的大小
        self.center_jitter_factor = center_jitter_factor  #{'train': 0., 'test': 4.5} 提取搜索区域之前应用于目标中心的抖动量，详见_get_jittered_box
        self.scale_jitter_factor = scale_jitter_factor  #{'train': 0., 'test': 0.5}  提取搜索区域之前应用于目标大小的抖动量
        self.crop_type = crop_type  #'inside_major' 搜索区域裁剪会移动/收缩，以完全适应图像的一个轴。
        self.mode = mode  #sequence  输出帧以外的其他维度
        self.max_scale_change = max_scale_change #1.5 执行裁剪时允许的最大缩放变化
        self.stride = stride
        self.label_function_params = label_function_params  #标签生成过程的参数  {'feature_sz': 18, 'sigma_factor': output_sigma, 'kernel_sz': 1}
        self.center_sampling_radius = center_sampling_radius  #1.0  中心采样半径
        self.use_normalized_coords = use_normalized_coords  #True  使用规范化坐标
    ...
    def _get_jittered_box(self, box, mode):
        ...
        #这些偏移量与scale_jitter_factor，center_jitter_factor 息息相关，原tomp中train是不进行抖动的
        jittered_size = box[2:4] * torch.exp(torch.randn(2) * self.scale_jitter_factor[mode])  #抖动后目标bbox的大小
        max_offset = (jittered_size.prod().sqrt() * torch.tensor(self.center_jitter_factor[mode]).float())  # 最大偏移量
        jittered_center = box[0:2] + 0.5 * box[2:4] + max_offset * (torch.rand(2) - 0.5)   #目标中心的抖动量
        out = torch.cat((jittered_center - 0.5 * jittered_size, jittered_size), dim=0)
        return out

    def _generate_label_function(self, target_bb):
        """ Generates the gaussian label function centered at target_bb 生成以target_bb为中心的高斯标签函数,score map的形式
        args:
            target_bb - target bounding box (num_images, 4)
        ...
        gauss_label = prutils.gaussian_label_function(target_bb.view(-1, 4),
                                                      self.label_function_params['sigma_factor'],    #0.05
                                                      self.label_function_params['kernel_sz'],       #1
                                                      self.label_function_params['feature_sz'],      #18
                                                      self.output_sz,                                #288
                                                      end_pad_if_even=self.label_function_params.get(
                                                          'end_pad_if_even', True))  #[2,18,18]
        '''
        产生以目标中心高斯分布的分布图[18,18]。
        高斯分布：越接近中心，取值越大，越远离中心，取值越小，因此计算平均值时，只需将“中心点”作为原点，
                其他点按照其在正态曲线上的位置，分配权重，就可以得到一个加权平均值。
        '''
        return gauss_label
    ...
    def __call__(self, data: TensorDict):
        ...
        args:
            data - The input data, should contain the following fields:
                'train_images', test_images', 'train_anno' (训练image标注), 'test_anno' (测试image标注)
        returns:
            TensorDict - output data block with following fields:
                'train_images', 'test_images', 'train_anno', 'test_anno', 'test_proposals', 'proposal_density', 'gt_density',
                'test_label' (optional), 'train_label' (optional), 'test_label_density' (optional), 'train_label_density' (optional)
        ...
        if self.transform['joint'] is not None:
            data['train_images'], data['train_anno'] = self.transform['joint'](image=data['train_images'],  #调用transformers的TransformBase.__call__()函数,每个不同的变换调用一次
                                                                               bbox=data['train_anno'])
            # train_image，test_image 进行图像翻转变化
        ...
        for s in ['train', 'test']:
            ...
            # Add a uniform noise to the center pos 抖动后的bbox标签，train_image的jittered_anno是gt_bbox大小，test_image的jittered_anno是抖动大小
            jittered_anno = [self._get_jittered_box(a, s) for a in data[s + '_anno']]

            #获取train_image和test_image裁剪和resize后的图片和boxes
            crops, boxes = prutils.target_image_crop(data[s + '_images'], jittered_anno, data[s + '_anno'],
                                                     self.search_area_factor, self.output_sz, mode=self.crop_type,
                                                     max_scale_change=self.max_scale_change)

            data[s + '_images'], data[s + '_anno'] = self.transform[s](image=crops, bbox=boxes, joint=False)
            #进行图像变换，train_image numpy->tensor,并且进行一些图像上的处理，test_image不变
        ...
        # Generate label functions 生成标签函数
        #目标为中心的高斯函数
        if self.label_function_params is not None:
            data['train_label'] = self._generate_label_function(data['train_anno'])
            data['test_label'] = self._generate_label_function(data['test_anno'])

        #目标大小编码:计算特征图上每一个位置距离bbox的归一化距离[1,1,4,18,18]
...
class LSTOMPProcessing(BaseProcessing):
    """ The processing class used for training ToMP that supports dense bounding box regression.
        用于训练支持密集边界框回归的ToMP的处理类  ,生成bbox的ltrb编码
    """
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['train', 'test']:
            ...
            #获取train_image和test_image裁剪和resize后的图片和boxes
            crops, boxes,shift_boxes,im_resize_factor = prutils.target_image_crop2(data[s + '_images'], jittered_anno, data[s + '_anno'],
                                                     self.search_area_factor, self.output_sz, mode=self.crop_type,
                                                     max_scale_change=self.max_scale_change)

            data[s + '_images'], data[s + '_anno'] = self.transform[s](image=crops, bbox=boxes, joint=False)
            data[s + '_shift_boxes'] = shift_boxes
            data[s + '_im_resize_factor'] = im_resize_factor
            #进行图像变换，train_image numpy->tensor,并且进行一些图像上的处理，test_image不变
