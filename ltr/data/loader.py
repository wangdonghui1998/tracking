...
class LTRLoader(torch.utils.data.dataloader.DataLoader):
    ...
    Note: The only difference with default pytorch DataLoader is that an additional option stack_dim is available to
            ...
            与默认pytorch数据加载器的唯一区别在于，额外的选项stack_dim可用于选择数据应沿哪个维度堆叠以形成批次。

    Arguments:
        dataset (Dataset): dataset from which to load the data.
        batch_size (int, optional): how many samples per batch to load
            (default: 1).
        shuffle (bool, optional): set to ``True`` to have the data reshuffled  True:可重新洗牌该数据
            at every epoch (default: False).
        sampler (Sampler, optional): defines the strategy to draw samples from 定义从中提取样本的策略
            the dataset. If specified, ``shuffle`` must be False.
        batch_sampler (Sampler, optional): like sampler, but returns a batch of 一次返回一批索引
            indices at a time. Mutually exclusive with batch_size, shuffle,
            sampler, and drop_last.
        num_workers (int, optional): how many subprocesses to use for data  加载的子进程的数量
            loading. 0 means that the data will be loaded in the main process.
            (default: 0)
        collate_fn (callable, optional): merges a list of samples to form a mini-batch. 合并样本列表形成小批次
        stack_dim (int): Dimension along which to stack to form the batch. (default: 0) 沿着堆叠以形成batch的维度
        pin_memory (bool, optional): If ``True``, the data loader will copy tensors
            into CUDA pinned memory before returning them.  True:数据加载器将复制tensors在返回他们之前，将其固定在CUDA内存中
        drop_last (bool, optional): set to ``True`` to drop the last incomplete batch,
            if the dataset size is not divisible by the batch size. If ``False`` and
            the size of dataset is not divisible by the batch size, then the last batch
            will be smaller. (default: False)  True:删除最后一个不完整的batch
