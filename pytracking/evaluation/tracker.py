...
def trackerlist(name: str, parameter_name: str, run_ids = None, display_name: str = None):
    ...
    if run_ids is None or isinstance(run_ids, int):
        run_ids = [run_ids]
    return [Tracker_two(name, parameter_name, run_id, display_name) for run_id in run_ids]
...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, run_id: int = None, display_name: str = None):
        ...
        env = env_settings()
        #result存放的位置，最好再加一个目录，为glass/otb/...
    ...
    def create_tracker(self, params):
        tracker = self.tracker_class(params)   #每个tracker的__init__.py中会写
    ...
    def run_sequence(self, seq, visualization=None, debug=None, visdom_info=None, multiobj_mode=None):
        ...
        args:
            ...
            multiobj_mode: Which mode to use for multiple objects.  用于多个对象的模式
        """
        params = self.get_parameters()  # 加载的依然是paramter/lstr/lstr
        ...
        # Get init information  得出初始帧得box
        ...
        if multiobj_mode == 'default' or is_single_object:  #进入这步
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        #储存跟踪结果{'target_bbox','time','segmentation','object_presence_score'}
        ...
        # print(init_info)  #init_info是之前就得出来得
        out = tracker.initialize(image, init_info)  #与跟踪器中的initialize函数联系起来
        if out is None:
            out = {}

        # prev_output：是上一帧跟踪的信息{'target_bbox','time','segmentation','object_presence_score'}
        prev_output = OrderedDict(out)  # 只执行一次
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            info = seq.frame_info(frame_num)  #生成跟踪的dict{}(空)
            info['previous_output'] = prev_output
            # out = tracker.track(image, info,prev_output)  #与跟踪器中的track函数联系起来
            out = tracker.track(image, info)
            prev_output = OrderedDict(out)    #执行很多次，保存的是上一帧的跟踪结果，跟踪结果从第二帧开始
...
class Tracker_two:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        # print(init_info)  #init_info是之前就得出来得
        out = tracker.initialize(image, init_info)  #与跟踪器中的initialize函数联系起来
        center = [out['target_bbox'][0] + out['target_bbox'][2] / 2,out['target_bbox'][1] + out['target_bbox'][3] / 2]
        delta_xy = [0,0]
        if out is None:
            out = {}
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            info = seq.frame_info(frame_num)  #生成跟踪的dict{}(空)
            info['previous_output'] = prev_output
            out = tracker.track(image, info, output)  #与跟踪器中的track函数联系起来
            # out = tracker.track(image, info)
            #获得跟踪框的center
            center =  [out['target_bbox'][0] + out['target_bbox'][2] / 2 ,out['target_bbox'][1] + out['target_bbox'][3] / 2]

            prev_output = OrderedDict(out)    #执行很多次，保存的是上一帧的跟踪结果，跟踪结果从第二帧开始
            delta_xy = [center[0] - output['center'][-1][0],center[1] - output['center'][-1][1]]
            _store_outputs(out,{'center':center, 'delta_xy':delta_xy, 'time': time.time() - start_time})
