...
class MultiResolutionExtractor(ExtractorBase):
    ...
    def extract(self, im, pos, scales, image_sz, return_patches=False):
        ...
        args:
            im: Image.
            pos: Center position for extraction.   提取的中心点位置
            scales: Image scales to extract features from.  从中提取特征的图像比例
            image_sz: Size to resize the image samples to before extraction.  提取特征之前将图像样本调整到的大小
        """
        if isinstance(scales, (int, float)):
            scales = [scales]
