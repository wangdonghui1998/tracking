...
from pytracking.utils.get_samples import get_samples,divide_train_test
import lstm_model.ts_model as ts_model


class ToMP_LSTR(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        # 新建一个可以存储历史deta x,deta y值的numpy数组
        self.list_deta_xy = np.zeros((0, 2))
    ...
    def track(self, image, info: dict = None,prev_output: dict = None) -> dict:
        ...
        # 存储θ结果
        prev_out = prev_output.get('target_bbox')
        vx = (output_state[0] + output_state[2] / 2) - (prev_out[0] + prev_out[2] / 2)
        vy = (output_state[1] + output_state[3] / 2) - (prev_out[1] + prev_out[3] / 2)
        self.list_deta_xy = np.append(self.list_deta_xy, [[vx,vy]], axis=0)
        return out
    ...
    def classify_target(self, sample_x: TensorList):
        ...
        with torch.no_grad():
            ...
            # fuse encoder and decoder features to one feature map  将编码器和解码器特征融合到一个特征图
    ...
    # 先把置信度在最大值周围 -0.4的找出来, 然后找局部最大值
    def detect_peaks(self, score):
        max_score1, max_disp1 = dcf.max2d(score)
        score = np.array(score.squeeze().cpu())
        peaks = np.zeros_like(score)
        score_sz = score.shape
        for i in range(score_sz[0]):
            for j in range(score_sz[1]):
                if score[i][j] <= max_score1 and score[i][j] >= max_score1 - 0.4:
                    peaks[i][j] = score[i][j]

        neighborhood = np.ones((4, 4))
        local_max = maximum_filter(peaks, footprint=neighborhood) == peaks
        background = (peaks == 0)
        eroded_background = binary_erosion(background, structure=neighborhood, border_value=1)
        detected_peaks = local_max ^ eroded_background
        return detected_peaks

    def dataintegration(self, data1=np.zeros(0), data2=np.zeros(0), data3=np.zeros(0), data4=np.zeros(0)):
        if len(data2) == 0 and len(data3) == 0 and len(data4) == 0:
            count = 1
        elif len(data3) == 0 and len(data4) == 0:
            count = 2
        elif len(data4) == 0:
            count = 3
        else:
            count = 4
        result = np.zeros((0, count))
        for i in range(len(data1)):
            if count == 1:
                result = np.append(result, [data1[i]])
                result = np.expand_dims(result, axis=1)
            elif count == 2:
                result = np.append(result, [[data1[i], data2[i]]], axis=0)
            elif count == 3:
                result = np.append(result, [[data1[i], data2[i], data3[i]]], axis=0)
            else:
                result = np.append(result, [[data1[i], data2[i], data3[i], data4[i]]], axis=0)
        return result
    ...
    def localize_advanced(self, scores, sample_pos, sample_scales):
        ...
        # if self.frame_num > 66:
        plt.matshow(scores.squeeze().cpu(),cmap='viridis')
        plt.title(str(self.frame_num))
        vis.matplot(plt)
        ...
        # 获取局部最大值
        peaks = self.detect_peaks(scores)
        # if self.frame_num>60:
        plt.matshow(peaks, cmap='viridis')
        plt.title(str(self.frame_num))
        vis.matplot(plt)

        # 如果peaks中为1的点不止有一个且不是前五帧，启动lstm
        peaks_count = np.count_nonzero(peaks)
        if peaks_count > 1 and self.frame_num > 5:
            output = {'location': np.zeros((0,2), dtype=int),
                    'deta_xy': np.zeros((0,2))}
            # 获取局部最大值的位置
            local_max = np.argwhere(peaks == 1)

            # 开启lstm
            print("self.frame_num=", self.frame_num)
            # lstm_data = self.dataintegration(self.list_deta_xy)
            ts = ts_model.ts_model()
            train_data, test_data = divide_train_test(self.list_deta_xy)
            n_preds = ts.fit_transform(train_data, test_data)
            preds = n_preds[-1]  # 只取最后一个预测值

            # 获取现在score-map中的所有deta_x,deta_y
            for i in range(0, local_max.shape[0]):
                deta_x = local_max[i][0] - pre_loc
                deta_y = local_max[i][1] - pre_loc
                output['location'] = np.append(output['location'], [[local_max[i][0],local_max[i][1]]], axis=0)
                output['deta_xy'] = np.append(output['deta_xy'], [[deta_x,deta_y]], axis=0)

            # 获取与preds最接近的deta_xy数组中的值
            local_score = output['deta_xy'].flat[np.abs(output['deta_xy'] - preds).argmin()]

            # 获取n_theta中的这个值在scores中的数值和位置，赋值给max_score1,max_disp1
            index = np.where(output['deta_xy'] == local_score)[0][0]

            max_disp1 = torch.from_numpy(output['location'][index]).unsqueeze(0)

            max_score1 = scores.squeeze()[max_disp1[0][0]][max_disp1[0][1]].unsqueeze(0)
