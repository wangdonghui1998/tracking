...
class LSATOM(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self.pos = torch.Tensor([state[1] + (state[3] - 1)/2, state[0] + (state[2] - 1)/2])   #目标中心点位置
        self.target_sz = torch.Tensor([state[3], state[2]])    #目标大小
    ...
    def track(self, image, info: dict = None, prev_output:dict=None) -> dict:
        ...
        test_x,sample_coords = self.extract_processed_sample(im, self.pos, sample_scales, self.img_sample_sz)
    ...
    def extract_processed_sample(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor) -> (TensorList, TensorList):
        x,patch_coords = self.extract_sample(im, pos, scales, sz)
        return self.preprocess_sample(self.project_sample(x)),patch_coords
    ...
    def get_samples(self, dataset, in_seq_len, pred_len, device):
        ...
        for i in range(0, dataset.shape[0] - in_seq_len, step):
            ...
            targetdata = torch.tensor([item.cpu().detach().numpy() for item in targetdata], dtype=torch.float).to(
                device)
