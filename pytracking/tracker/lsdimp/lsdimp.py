...
class LSDiMP(BaseTracker):
    ...
    def initialize(self, image, info :dict ) -> dict:
        ...
        #The LSTM network
        self.lstm_net = CNN_LSTM(in_channels=2, out_channels=32, kernel_size=3, hidden_size=64, num_layers=1, output_size=2).to(self.params.device)
    ...
    def track(self, image, info:dict=None, prev_output:dict=None) -> dict :
        ...
        # Extract backbone features  sample_coords：采样块的左上角和右下角坐标
        ...
        # Location of sample
        sample_pos, sample_scales = self.get_sample_location(sample_coords)   #sample_pos : 采样中心点的位置坐标  sample_scales: 裁剪区域相对于288的比例
        ...
        # Localize the target
        translation_vec, scale_ind, s, flag = self.localize_target(scores_raw, sample_pos, sample_scales, self.center)
        new_pos = sample_pos[scale_ind,:] + translation_vec   #新的中心点
        ...
        if update_flag and self.params.get('update_classifier', False):
            ...
            # Create target_box and label for spatial sample  为空间样本创建target_box和标签
            target_box = self.get_iounet_box(self.pos, self.target_sz, sample_pos[scale_ind,:], sample_scales[scale_ind])  #self.pos 上一帧

            # Update the classifier model
            self.update_classifier(train_x, target_box, learning_rate, s[scale_ind,...])

        # Set the pos of the tracker to iounet pos  将跟踪器的位置设置为iounet pos
    ...
    def get_sample_location(self, sample_coord):
        """Get the location of the extracted sample. 获取提取样本的中心点位置。 """
    ...
    def localize_target(self, scores, sample_pos, sample_scales, center = None):
        ...
        if center is not None:
            # plt.matshow(scores.squeeze().cpu().detach().numpy(), cmap='viridis')
            # plt.title("dimp_scores" + str(self.frame_num))
            # vis.matplot(plt)
            # print()

            # 生成以lstm预测出来的center值为中心的Gaussian分布
            center = torch.min(center, self.feature_sz)
            sigma_factor = 0.05546975541855379
            sigma = sigma_factor * (self.feature_sz+1).prod().sqrt().item()
            lstm_score_map = self.gauss_2d(sz=self.feature_sz+1 ,sigma = sigma, center=center)  # 基于预测出来的位置的guass
            lstm_score_map = lstm_score_map.to(self.params.device)

            # plt.matshow(lstm_score_map.squeeze().cpu().detach().numpy(), cmap='viridis')
            # plt.title("lstm_scores" + str(self.frame_num))
            # vis.matplot(plt)
            # print()

            scores = 0.3 * lstm_score_map +  0.7 * scores   #将lstm score 和 dimp score 加权融合 ，prdimp的需要将score归一化到（-1 ，1）范围内

            # plt.matshow(scores.squeeze().cpu().detach().numpy(), cmap='viridis')
            # plt.title("fianl_scores" + str(self.frame_num))
            # vis.matplot(plt)
            # print()
        ...
        # Compute translation vector and scale change factor 计算平移矢量和比例变化因子
    ...
    def localize_advanced(self, scores, sample_pos, sample_scales):
        ...
        sz = scores.shape[-2:]  #获取score map的宽和高 [19,19]

        score_sz = torch.Tensor(list(sz))  #score_sz [19,19]
        output_sz = score_sz - (self.kernel_size + 1) % 2   #[18,18]
        score_center = (score_sz - 1)/2

        scores_hn = scores
        if self.output_window is not None and self.params.get('perform_hn_without_windowing', False):  #false
            scores_hn = scores.clone()
            scores *= self.output_window


        max_score1, max_disp1 = dcf.max2d(scores)
        _, scale_ind = torch.max(max_score1, dim=0)  #0
        sample_scale = sample_scales[scale_ind]   #采样相对于288的大小 tensor(1.5955)
        max_score1 = max_score1[scale_ind]   # 相当于去掉一个[] 0.6358
        max_disp1 = max_disp1[scale_ind,...].float().cpu().view(-1)  # 相当于去掉[], [9，9]
        target_disp1 = max_disp1 - score_center  #目标相对于中心的偏移量
        translation_vec1 = target_disp1 * (self.img_support_sz / output_sz) * sample_scale  # 相对于上一帧的偏移向量

        if max_score1.item() < self.params.target_not_found_threshold:   #判断最大值点是否小于阈值（0.25）
            return translation_vec1, scale_ind, scores_hn, 'not_found'
        if max_score1.item() < self.params.get('uncertain_threshold', -float('inf')):  # 判断最大值点是否小于不确定的阈值
        ...
        # Mask out target neighborhood  遮蔽目标邻域
        target_neigh_sz = self.params.target_neighborhood_scale * (self.target_sz / sample_scale) * (output_sz / self.img_support_sz)  # 2.2 * 目标相对于采样区域内的大小 *1/16  目标box的2.2倍 映射回scoremap   —————— 这一行很有用

        tneigh_top = max(round(max_disp1[0].item() - target_neigh_sz[0].item() / 2), 0)   #目标邻域的上边界
        tneigh_bottom = min(round(max_disp1[0].item() + target_neigh_sz[0].item() / 2 + 1), sz[0])   #目标邻域的下边界
        tneigh_left = max(round(max_disp1[1].item() - target_neigh_sz[1].item() / 2), 0)   #目标邻域的左边界
        tneigh_right = min(round(max_disp1[1].item() + target_neigh_sz[1].item() / 2 + 1), sz[1])   #目标邻域的右边界
        scores_masked = scores_hn[scale_ind:scale_ind + 1, ...].clone()
        scores_masked[...,tneigh_top:tneigh_bottom,tneigh_left:tneigh_right] = 0    # scroe map 空间将邻域范围内的数值都赋值成0 ，遮蔽目标邻域

        # Find new maximum
        max_score2, max_disp2 = dcf.max2d(scores_masked)  #找到新的最大值
        max_disp2 = max_disp2.float().cpu().view(-1)
        target_disp2 = max_disp2 - score_center
        translation_vec2 = target_disp2 * (self.img_support_sz / output_sz) * sample_scale  #原图坐标空间，当前帧相对于上一帧的偏移向量

        prev_target_vec = (self.pos - sample_pos[scale_ind,:]) / ((self.img_support_sz / output_sz) * sample_scale)  #

        # Handle the different cases 处理不同的情况
        if max_score2 > self.params.distractor_threshold * max_score1:    #判断 当前最大值 > 0.8 * max_score1吗
    ...
    def update_state(self, new_pos, new_scale = None):
        ...
        if new_scale is not None:
            self.target_scale = new_scale.clamp(self.min_scale_factor, self.max_scale_factor)  #将input张量每个元素的值压缩到区间 [min,max],并返回结果到一个新张量。
            self.target_sz = self.base_target_sz * self.target_scale

        # Update pos 更新坐标
        inside_ratio = self.params.get('target_inside_ratio', 0.2)
        inside_offset = (inside_ratio - 0.5) * self.target_sz    #偏移量
        self.pos = torch.max(torch.min(new_pos, self.image_sz - inside_offset), inside_offset)


    def get_iounet_box(self, pos, sz, sample_pos, sample_scale):
        """All inputs in original image coordinates.    原始图像坐标中的所有输入。
        Generates a box in the cropped image sample reference frame, in the format used by the IoUNet.  以IoUNet使用的格式在裁剪图像样本参考帧中生成一个框。 """
    ...
    def draw_deltaxy(self,lstm_target,lstm_pred):
        vis = visdom.Visdom(port=8098)
