...
class ToMP(BaseTracker):
    ...
    def track(self, image, info: dict = None, prev_output:dict=None) -> dict:
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, self.get_centered_sample_pos(),
                                                                      ...
                                                                      self.img_sample_sz)    #dict 类型[1,1024,18,18]
        ...
        # im = im_patches.squeeze().swapaxes(0, 1)
        # im = im.swapaxes(1, 2) / 255
        # plt.imshow(im.cpu().detach().numpy(), cmap='viridis')
        # plt.title("image" + str(self.frame_num))
        # vis.matplot(plt)
        # plt.matshow(s.squeeze().cpu().detach().numpy(), cmap='viridis')
        # plt.title("scores" + str(self.frame_num))
        # vis.matplot(plt)
    ...
    def classify_target(self, sample_x: TensorList):
        ...
        with torch.no_grad():
            ...
            test_feat = self.net.head.extract_head_feat(sample_x)    #头部特征提取 [1,256,18,18]
